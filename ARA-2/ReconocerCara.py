...
while(cap.isOpened()):
	...
	width = cap.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH)   # Ancho video
	height = cap.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT) # Altura video
	...
	cv2.line(img, (0, int(height/2)), (int(width), int(height/2)), (0,255,0), 2)
	cv2.line(img, (int(width/2), 0), (int(width/2), int(height)), (0,255,0), 2)		
