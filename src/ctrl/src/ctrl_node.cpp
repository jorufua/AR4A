...
/////////////////////////////// Create Angles message
	...
	ang.angular.z = yaw*(180/M_PI);
        if (t>0 && t<=10){
            des.pose.position.x=2;//cos(0.125*t); //0.5;//cos(((t)*8)*(M_PI/180));
            des.pose.position.y=0;//0.5*sin(0.25*t);// 0.5;//sin(((t)*8)*(M_PI/180));
            des.header.stamp = ros::Time::now();
            pub_ang.publish(ang);
            }
        else if (t>10 && t<=20) {
            des.pose.position.x=2;//cos(0.125*t); //0.5;//cos(((t)*8)*(M_PI/180));
            des.pose.position.y=2;//0.5*sin(0.25*t);// 0.5;//sin(((t)*8)*(M_PI/180));
            des.header.stamp = ros::Time::now();
            pub_ang.publish(ang);
        }
        else if (t>20 && t<=30) {
            des.pose.position.x=0;//cos(0.125*t); //0.5;//cos(((t)*8)*(M_PI/180));
            des.pose.position.y=2;//0.5*sin(0.25*t);// 0.5;//sin(((t)*8)*(M_PI/180));
            des.header.stamp = ros::Time::now();
            pub_ang.publish(ang);
        }
